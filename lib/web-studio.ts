...
function renderProject(spec: WebStudioSpec) {
  ...
  return {
    ...
    ".gitignore": ".next\nnode_modules\n.env*\n",
    "README.md": `# ${spec.brandName}\n\nGenerated by Khasroy Web Studio. Lead forms are stored in Khasroy Web Studio database through a server-only site token.\n`,
...
async function createVercelProject(token: string, desiredName: string, repoFullName: string, query: string) {
  ...
  const first = await attempt(asciiSlug(desiredName));
  if (first.response.ok) return first;
  if (first.response.status !== 409) throw new Error(`vercel_project_${first.response.status}:${vercelError(first.payload)}`);
  const second = await attempt(`${asciiSlug(desiredName).slice(0, 48)}-${Date.now().toString(36).slice(-5)}`);
  if (!second.response.ok) throw new Error(`vercel_project_${second.response.status}:${vercelError(second.payload)}`);
  return second;
}

async function configureProjectEnv(args: { token: string; project: string; query: string; projectSlug: string; siteToken: string }) {
  const response = await fetch(`https://api.vercel.com/v10/projects/${encodeURIComponent(args.project)}/env?upsert=true${args.query ? `&${args.query.slice(1)}` : ""}`, {
    method: "POST",
  ...
  if (!response.ok) throw new Error(`vercel_env_${response.status}:${vercelError(payload)}`);
}
...
async function publishVercel(args: { token: string; repoFullName: string; repoName: string; login: string; projectName: string; projectSlug: string; siteToken: string }) {
  const teamId = process.env.KHASROY_VERCEL_TEAM_ID?.trim();
  const query = teamId ? `?teamId=${encodeURIComponent(teamId)}` : "";
  const created = await createVercelProject(args.token, args.projectName, args.repoFullName, query);
  const projectName = String(created.payload?.name || created.name);
  const projectId = String(created.payload?.id || "");
  await configureProjectEnv({ token: args.token, project: projectId || projectName, query, projectSlug: args.projectSlug, siteToken: args.siteToken });
  ...
  return { vercelProjectId: projectId || undefined, deployUrl, deploymentId: deploymentId || undefined, deploymentReady, httpVerified, databaseLeads: true };
}
