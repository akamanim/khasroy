...
export async function resolveAISecrets(ownerKey: string): Promise<ResolvedAISecrets> {
  const [groq, openai, gemini, kimi] = await Promise.all([
    resolveSecret(ownerKey, "groq", [process.env.GROQ_API_KEY]).catch(
      () => process.env.GROQ_API_KEY?.trim() || "",
    ),
    resolveSecret(ownerKey, "openai", [process.env.OPENAI_API_KEY]).catch(
      () => process.env.OPENAI_API_KEY?.trim() || "",
    ),
    resolveSecret(ownerKey, "gemini", [process.env.GEMINI_API_KEY]).catch(
      () => process.env.GEMINI_API_KEY?.trim() || "",
    ),
    resolveSecret(ownerKey, "kimi", [process.env.MOONSHOT_API_KEY, process.env.KIMI_API_KEY]).catch(
