...
export function externalTeacherModel(provider: ExternalTeacher) {
  switch (provider) {
    case "openai":
      return env("KHASROY_OPENAI_MODEL") || "gpt-5.6-terra";
    case "gemini":
      return env("KHASROY_GEMINI_MODEL") || "gemini-3.5-flash";
    case "kimi":
...
function normalizedResponse(
  ...
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      "x-khasroy-ai-provider": provider,
    },
  });
}
...
async function openAIRequest(args: RunArgs): Promise<TeacherRun> {
  ...
  if (!key) {
    ...
    return {
      response: normalizedResponse(provider, 503, data),
      data,
      provider,
      model,
      latencyMs: 0,
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model,
        instructions: args.systemContent,
        input: args.history,
        max_output_tokens: args.maxCompletion,
      }),
      cache: "no-store",
    ...
    return {
      response: normalizedResponse(provider, response.status, data),
      data,
      provider,
    ...
    const data = errorData(
      error instanceof Error ? error.message : "OpenAI unavailable",
      "network",
      "network_error",
    );
    return {
...
async function kimiRequest(args: RunArgs): Promise<TeacherRun> {
  ...
  try {
    ...
    const data = errorData(
      error instanceof Error ? error.message : "Kimi unavailable",
      "network",
      "network_error",
    );
    return {
...
export async function runExternalTeacher(
  provider: ExternalTeacher,
  args: RunArgs,
): Promise<TeacherRun> {
