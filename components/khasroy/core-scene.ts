...
export function createCoreScene(
  ...
  const circuits = new T.LineSegments(
    geometry(new T.BufferGeometry().setFromPoints(traces)),
    material(new T.LineBasicMaterial({ color: 0xffa65b, opacity: .64, ...additive })),
  );
  root.add(circuits);
